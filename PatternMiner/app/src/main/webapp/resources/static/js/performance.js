const DEFAULT_OPTIONS = {order: 2, precision: 4, period: null};
...
function exponential(data, options) {
...
function medianTime(data) {
    ...
    jQuery.each(data, function (i, val) {
        let m = median(unpack(val, 'timeInMillis'));
        result.push([Number(i), m])
    });
    return result;
}
...
function range(start, end, n) {
    var l = (end - start) / (n - 1);
    return Array.from(new Array(n), (x, i) => round(start + (l * i), 4));
}

function create3DPlot(stats) {
    let data3d = [];
    let statsCount = Object.keys(stats).length - 1;
    let counter = 0;

    jQuery.each(stats, function (i, val) {
        let color = 'hsl(' + (240.0 * (counter++) / statsCount) + ', 80, 50)';

        let trace = {
            x: unpack(val, 'sequenceCount'),
            y: unpack(val, 'minSup'),
            z: unpack(val, 'timeInMillis'),
            mode: 'markers',
            marker: {
                size: 2,
                color: color,
    ...
    var layout = {
        dragmode: true,
        title: '3D Plot of SequenceCount-MinimalSupport-Time',
        scene: {
            xaxis: {
                title: 'Sequnce Count',
                type: 'log',
    ...
    Plotly.newPlot('stats3D', data3d, layout);
}


function createReg(trace) {
    let reg = polynomial(transpose([trace.x, trace.y]), DEFAULT_OPTIONS);

    let xs = range(Math.min(...trace.x), Math.max(...trace.x), 2);
    let ys = xs.map(function (x) {
        let y = reg.equation[0] * (x ** 2) + reg.equation[1] * x + reg.equation[2];
        if (y < 0) {
            y = 0
        }
        ;
...
function create2DPlot_MinSupport(stats) {
    let dataMinSup = [];
    let statsCount = Object.keys(stats).length - 1;
    let counter = 0;
    ...
    let layout = {
        dragmode: true,
        title: '2D Plot of MinimalSupport-Time',
        xaxis: {
            title: 'Minimal Support',
            type: 'log',
            autorange: true
    ...
    Plotly.newPlot('statsMinSup', dataMinSup, layout);
}


function create2DPlot_SeqCount(stats) {
    let dataSeqCount = [];
    ...
    let layout = {
        title: '2D Plot of SequenceCount-Time',
        xaxis: {
    ...
    Plotly.newPlot('statsSeqCount', dataSeqCount, layout);
}

// on document ready init plots
$(document).ready(function () {
    let stats = JSON.parse($('#mainStatsJSON').val());
    create2DPlot_MinSupport(stats);
    create2DPlot_SeqCount(stats);
    create3DPlot(stats);
});