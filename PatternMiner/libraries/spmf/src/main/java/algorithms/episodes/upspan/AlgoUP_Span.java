package algorithms.episodes.upspan;
...
public class AlgoUP_Span {
     ...
     * Run this algorithm
     *
     * @param inputFile           an input file path
     * @param outputFile          an output file path
     * @param minimumUtility      a minimum utility threshold (percentage as double value)
     * @param maximumTimeDuration a minimum time duration
     * @param outputSingleEvents  if true, the single events will be added to the output file.
     */
        ...
        try {
            ...
            for (int i = 0; i < FreEP.size(); i++) {
                if (EPCount.get(i) >= (minUtility * totalUtilityinAllSequence)) //
                {
                    ...
                    String episodeName = FreEP.get(i).toString();
                    // if
