...
public class UPTreePlus {
    ...
    public void addLocalTransaction(List<Integer> localPath, int pathUtility,
        ...
        UPNodePlus currentlocalNode = root;
        int i = 0;
        int RemainingUtility = 0;
    ...
    private UPNodePlus insertNewNode(UPNodePlus currentlocalNode, int item, int nodeUtility, int MinimalNodeUtility) {
        ...
        // header table
        UPNodePlus localheadernode = mapItemNodes.get(item);
        if (localheadernode == null) { // there is not
            ...
            UPNodePlus lastNode = mapItemLastNode.get(item);
     ...
     *
     * @param mapItemToEstimatedUtility the Utilities of each item (key: item value: TWU or path
     *                                  utility)
     */
    ...
    /**
     * Get a string representation of this tree (mainly, for debugging)
     *
