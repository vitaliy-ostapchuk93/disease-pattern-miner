...
/**
 * This is an implementation of the "FCHM_allconfidence" algorithm for High-Utility Itemsets Mining
 ...
 * @see UtilityListFCHM_all_confidence
 * @see Element
 */
public class AlgoFCHM_all_confidence {
     ...
     * Default constructor
     */
    public AlgoFCHM_all_confidence() {
    ...
    public void runAlgorithm(String input, String output, int minUtility, double minAllconfidence) throws IOException {
        ...
        List<UtilityListFCHM_all_confidence> listOfUtilityListFCHM2s = new ArrayList<UtilityListFCHM_all_confidence>();
        // CREATE A MAP TO STORE THE UTILITY LIST FOR EACH ITEM.
        // Key : item    Value :  utility list associated to that item
        Map<Integer, UtilityListFCHM_all_confidence> mapItemToUtilityListFCHM2 = new HashMap<Integer, UtilityListFCHM_all_confidence>();
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
                    ...
                    UtilityListFCHM_all_confidence UtilityListFCHM2OfItem = mapItemToUtilityListFCHM2.get(pair.item);
        ...
        //===== initialize the max_subset of 1-itemset =====
        for (int i = 0; i < listOfUtilityListFCHM2s.size(); i++) {
            listOfUtilityListFCHM2s.get(i).max_subset = listOfUtilityListFCHM2s.get(i).elements.size();
        }
        ...
        fchm(itemsetBuffer, 0, null, listOfUtilityListFCHM2s, minUtility, minAllconfidence);
     ...
     * @param minAll_confidence
     * @throws IOException
     */
    private void fchm(int[] prefix,
                      int prefixLength, UtilityListFCHM_all_confidence pUL, List<UtilityListFCHM_all_confidence> ULs, int minUtility, double minAll_confidence)
            throws IOException {
        ...
        for (int i = 0; i < ULs.size(); i++) {
            ...
            if (X.sumIutils >= minUtility) {
                // save to file
                writeOut(prefix, prefixLength, X.item, X.sumIutils, X.getAll_confidence());
            }
            ...
            if (X.sumIutils + X.sumRutils >= minUtility) {
                ...
                List<UtilityListFCHM_all_confidence> exULs = new ArrayList<UtilityListFCHM_all_confidence>();
                ...
                for (int j = i + 1; j < ULs.size(); j++) {
                    ...
                    UtilityListFCHM_all_confidence temp = construct(pUL, X, Y, minUtility);
                    if (temp != null && temp.getAll_confidence() >= minAll_confidence) {
                        exULs.add(temp);
                    }
                }
                ...
                fchm(itemsetBuffer, prefixLength + 1, X, exULs, minUtility, minAll_confidence);
            }
        }
        MemoryLogger.getInstance().checkMemory();
    }
