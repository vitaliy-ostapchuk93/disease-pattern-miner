...
public class AlgoSPAM {
    ...
    private final static Logger LOGGER = Logger.getLogger(AlgoSPAM.class.getName());
    /**
    ...
    private void spam(String input, double minsupRel) throws IOException {
        ...
        LOGGER.info("Building Bitmap representation.");
        ...
        LOGGER.info("Bitmap representation finished.");
        ...
        LOGGER.info("MinSup calc: " + minsup);
        LOGGER.info("Building vertical DB representation.");
...
//						determineSection(bitindex - previousBitIndex);  // register the sequence length for the bitmap
        ...
        LOGGER.info("Vertical DB representation finished.");
        LOGGER.info("Removing infrequent items.");
        ...
        LOGGER.info("Perform depth-first search of remaining items.");
        ...
        int itemsCount = verticalDB.size();
        int counter = 0;
        for (Entry<Integer, Bitmap> entry : verticalDB.entrySet()) {

            counter++;
            LOGGER.info("Searching item " + (counter) + " | " + itemsCount);
