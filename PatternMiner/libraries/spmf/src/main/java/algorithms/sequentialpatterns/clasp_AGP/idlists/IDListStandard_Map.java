package algorithms.sequentialpatterns.clasp_AGP.idlists;

import algorithms.sequentialpatterns.clasp_AGP.dataStructures.patterns.Pattern;
import algorithms.sequentialpatterns.clasp_AGP.tries.Trie;
...
//get minimum of sizes of two IDList:
        int size = (((IDListStandard_Map) idList).getSequencePositionsEntries().size() > this.getSequencePositionsEntries().size())
                ? this.getSequencePositionsEntries().size() : ((IDListStandard_Map) idList).getSequencePositionsEntries().size();
...
//       Map<Integer, List<Position>> intersection = new HashMap<Integer, List<Position>>(((IDListStandard_Map) idList).getSequencePositionsEntries().size());
     ...
     * appearances that correspond to a same sequence in two different patterns
     *
     * @param key                                Sequence identifier of the sequence where we want to check if
     *                                           it exists the pattern
