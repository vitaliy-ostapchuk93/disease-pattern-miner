package algorithms.sequentialpatterns.fournier2008_seqdim.multidimensionalpatterns;
...
import algorithms.frequentpatterns.aprioriTID.AlgoAprioriTID;
import algorithms.frequentpatterns.aprioriTIDClose.AlgoAprioriTIDClose;
...
public class AlgoDim {
    ...
    public MDPatterns runAlgorithm(MDPatternsDatabase mdPatDatabase, double minsupp) throws IOException {
        ...
        if (findClosedPatternsWithCharm) {
            ...
            patterns.itemset_array_integers_with_tids.Itemsets closedItemsets = apriori.runAlgorithm(database, minsupp, null);

            // Convert patterns found by AprioriClose into MDPatterns

            // for each level
            for (List<patterns.itemset_array_integers_with_tids.Itemset> itemsets : closedItemsets.getLevels()) {
                // for each pattern of that level
                for (patterns.itemset_array_integers_with_tids.Itemset itemset : itemsets) {
                    // convert to a md-pattern
            ...
            patterns.itemset_array_integers_with_tids.Itemsets closedItemsets = apriori.runAlgorithm(database, minsupp);
     ...
     */
    private MDPattern convertItemsetToPattern(patterns.itemset_array_integers_with_tids.Itemset itemset) {
